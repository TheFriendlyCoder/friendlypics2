...
def get_version_number(project):
    ...
    retval += ".dev"
    retval += os.environ.get('TRAVIS_BUILD_NUMBER', "0")
...
def _load_data_files(properties):
    """Helper method that generates setuptools data for non-source data files

    example use:

        setup (package_data=_load_data_files(PROJECT),...)
    ...
    Returns:
        dict: setuptools compatible descriptor for the non-source data
    """
    if "DATA" not in properties:
        return dict()
    return {PROJECT["NAME"]: PROJECT["DATA"]}
...
setup(
    ...
    package_dir={'': 'src'},
    package_data=_load_data_files(PROJECT),
